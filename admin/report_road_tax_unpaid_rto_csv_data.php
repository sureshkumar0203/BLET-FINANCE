...
foreach($db->fetchOrder('rto_office',"id='$_REQUEST[rto_office]'","") as $allrto){
	
	$is_exist = '';
	# Check the vehicle RTO Office wise
	foreach($db->fetchOrder('vehicle_registration',"rto_office='$allrto[id]'","") as $all_vehicle){
		
		$is_exist = $db->getDataFromTable("roadtax_pending", "id", "vehicle_id='$all_vehicle[id]' And (tax_to_dt between '$year_start_date' And '$year_end_date')");
		if($is_exist != ""){
			break;
		}
	}
